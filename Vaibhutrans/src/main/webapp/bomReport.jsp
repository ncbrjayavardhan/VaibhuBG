...
<head>
    ...
    <title>BOM Bank Statement Report - Vaibhutrans</title>
    <!-- Google Fonts -->
    ...
    <style>
        :root {
            ...
            --tally-gradient: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);
...
<jsp:include page="navbar.jsp" />
        ...
        <div class="report-card">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center header-title-container gap-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h3 class="report-title mb-0">BOM Bank Statement</h3>
                        <p class="text-muted small mb-0">Manage, filter, and export Bank of Maharashtra transaction reports</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-floating">
                            <select id="filterAccountNo" class="form-select" onchange="applyFilters()">
                                <option value="">All Accounts of BOM</option>
                            </select>
            ...
            <div class="actions-bar d-flex flex-wrap align-items-center justify-content-between mb-3 gap-2">
                ...
                <div class="d-flex flex-wrap gap-2">
                    <button type="button" class="btn btn-gradient-tally btn-sm" onclick="exportTallyLedgerFormat()">
                        <i class="fa fa-file-excel"></i> Tally Ledger
                    </button>
                    ...
                    <tbody>
                        <% 
						    ...
						    if (list != null && !list.isEmpty()) {
						        for (BomTransaction tx : list) {
						            ...
						            if (bankName == null || !"BOM".equalsIgnoreCase(bankName.trim())) {
						                continue;
						            }
						            bomCount++;
						            ...
						            String tallyLedger = tx.getTallyLedger() != null ? tx.getTallyLedger().trim() : "";
                                    String txNarration = tx.getNarration() != null ? tx.getNarration().trim() : "";
						%>
						<tr data-account-no="<%= accNo %>" data-tallyledger="<%= tallyLedger %>" data-narration="<%= txNarration %>">
                        ...
                        <tr>
                            <td colspan="10" class="text-center py-5 text-muted">
                                ...
                                No Bank of Maharashtra (BOM) transactions found.
                            </td>
                        </tr>
                        <% } %>
                    </tbody>
    ...
    var tallyLedgerMasterList = [
        <% 
            List<String> tallyHeads = (List<String>) request.getAttribute("tallyLedgerHeads");
            if (tallyHeads != null) {
                for (int i = 0; i < tallyHeads.size(); i++) {
                    String head = tallyHeads.get(i).replace("\"", "\\\"");
        %>
            "<%= head %>"<%= (i < tallyHeads.size() - 1) ? "," : "" %>
        <% 
                }
            }
        %>
        ...
        function formatToTallyDate(dateObj) {
        ...
        function populateBomAccountDropdown() {
        ...
        /* Helper to normalize ledger names & fallback N/A to Suspence Account */
        
        /*  function normalizeLedgerName(name) {
        if (!name || name.trim() === "" || name.trim().toUpperCase() === "N/A") {
            return "Suspence Account";
        }
        
        var cleanName = name.trim();
        var lowerInput = cleanName.toLowerCase();

        // 1. Check exact / case-insensitive match against table records
        for (var i = 0; i < tallyLedgerMasterList.length; i++) {
            var masterHead = tallyLedgerMasterList[i];
            if (masterHead.toLowerCase() === lowerInput) {
                return masterHead;
            }
        }

        // 2. Check substring / keyword match against table records
        for (var j = 0; j < tallyLedgerMasterList.length; j++) {
            var masterHead = tallyLedgerMasterList[j];
            var masterLower = masterHead.toLowerCase();

            // Match if table head is contained in input or input is contained in table head
            if (lowerInput.indexOf(masterLower) !== -1 || masterLower.indexOf(lowerInput) !== -1) {
                return masterHead;
            }
        }

        // 3. Fallback to raw name if no master head matches
        return cleanName;
    }  */
          function normalizeLedgerName(name) {
            if (!name || name.trim() === "" || name.trim().toUpperCase() === "N/A") {
            ...
            if (lower.includes("babji kollipara") || lower.includes("k babji") || lower === "kbabji") {
                return "KBabji";
            }
            if (lower.includes("v and k ") || lower.includes("vk corptech")) {
                return "V & K CORPTECH PVT LTD";
            }
            if (lower.includes("indian oil")) {
                return "INDIAN OIL CORPORATION LIMITED";
            }
            if (lower.includes("vip w") || lower.includes("domkal")) {
                return "WEST BENGAL STATE ELECTRICITY DISTRIBUTION CO. LTD.";
            }
            if (lower.includes("indraprastha")) {
                return "INDRAPRASTHA GAS LIMITED";
            }
            if (lower.includes("tera soft")) {
                return "TERA SOFTWARE LIMITED - UPPCL";
            }
            
            return name;
        }  

        /* Tally Ledger Format Excel Export Linked via UTR */
        function exportTallyLedgerFormat() {
            var table = document.querySelector('table');
            ...
            var tallyHeaders = [
                ...
                'Voucher Type Name',
                'Voucher Date',
                'Bank Name / Debit Ledger',
                'Ledger Amount',
                'Ledger Name / Credit Ledger',
                'Voucher Narration'
            ];

            var tallyRows = [tallyHeaders];
            var monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
            
            // Dynamic naming trackers
            var selectedAccFilter = (document.getElementById('filterAccountNo') ? document.getElementById('filterAccountNo').value : '').trim();
            var exportedAccounts = new Set();
            var exportedMonthYears = new Set();

            for (var i = 0; i < rows.length; i++) {
                var r = rows[i];
                if (r.dataset.matched !== '1') continue;

                var rowAcc = (r.getAttribute('data-account-no') || '').trim();
                if (rowAcc) exportedAccounts.add(rowAcc);

                var rawDate = (r.cells[0].innerText || r.cells[0].textContent).trim();
                var particulars = (r.cells[2].innerText || r.cells[2].textContent).trim();
                var utr = (r.cells[3].innerText || r.cells[3].textContent).trim();
                var beneficiary = (r.cells[4].innerText || r.cells[4].textContent).trim();
                
                // Directly extracted attributes from TRANSACTIONS
                var tallyLedgerVal = (r.getAttribute('data-tallyledger') || '').trim();
                var txNarration = (r.getAttribute('data-narration') || '').trim();
                
                var debit = parseFloat((r.cells[7].innerText || r.cells[7].textContent).replace(/,/g, '')) || 0;
                var credit = parseFloat((r.cells[8].innerText || r.cells[8].textContent).replace(/,/g, '')) || 0;

                var parsedDate = parseDateString(rawDate);
                var voucherDate = parsedDate ? formatToTallyDate(parsedDate) : rawDate;

                if (parsedDate && !isNaN(parsedDate.getTime())) {
                    exportedMonthYears.add(monthNames[parsedDate.getMonth()] + "_" + parsedDate.getFullYear());
                }

                var companyName = "VAIBHU INFRA TECH INDIA PVT LTD (GST)";

                // Voucher Narration: particulars + txNarration if available, else particulars
                var voucherNarration = particulars + (txNarration !== "" ? " " + txNarration : "");

                // Check keywords in particulars and voucher narration
                var upperParticulars = particulars.toUpperCase();
                var upperNarration = voucherNarration.toUpperCase();

                var isTeraSoftware = upperNarration.indexOf("TERA") !== -1;

                var isBankCharges = upperParticulars.indexOf("GST") !== -1 || upperParticulars.indexOf("RTGS CHARGES") !== -1 || upperParticulars.indexOf("NEFT CHARGES") !== -1 || upperParticulars.indexOf("BANK CHARGES") !== -1;
                var isClixCapital = upperParticulars.indexOf("CLIX CAPITAL") !== -1;
                var isUgroCapital = upperParticulars.indexOf("UGRO CAPITAL") !== -1;
                var isOriental = upperParticulars.indexOf("ORIENTAL INSURANCE COMP") !== -1;
                var isIndraprastha = upperParticulars.indexOf("INDRAPRASTHA GAS LIMIT") !== -1 || upperParticulars.indexOf("INDRAPRASTHA") !== -1;
                var isLF = upperParticulars.indexOf("LF") !== -1; 
                var isSelfOrCash = upperParticulars.indexOf("SELF") !== -1 || upperParticulars.indexOf("CASH") !== -1;
                
                var isBomtl = upperParticulars.indexOf("60426266369") !== -1;
                var isBombvl = upperParticulars.indexOf("60519771229") !== -1;
                var isBomevvl = upperParticulars.indexOf("60515236729") !== -1;
                var isBomvl = upperParticulars.indexOf("60456756641") !== -1;
                var isBomtlnew = upperParticulars.indexOf("60457854626") !== -1;
                var isknl = upperParticulars.indexOf("60443153693") !== -1 || upperParticulars.indexOf("60443148152") !== -1 || upperParticulars.indexOf("60443153489") !== -1 || upperParticulars.indexOf("60443147307") !== -1;
                var isspac = upperParticulars.indexOf("60566871397") !== -1;
                var isdbintr = upperParticulars.indexOf("DEBIT INTEREST") !== -1;
                var isBankCommission = upperParticulars.indexOf("00918IPG") !== -1;

                // Check for internal contra account references (60555504645 or 60548252992)
                var isContraAcc1 = particulars.indexOf("60555504645") !== -1 || beneficiary.indexOf("60555504645") !== -1 || tallyLedgerVal.indexOf("60555504645") !== -1;
                var isContraAcc2 = particulars.indexOf("60548252992") !== -1 || beneficiary.indexOf("60548252992") !== -1 || tallyLedgerVal.indexOf("60548252992") !== -1;
                var isContra = isContraAcc1 || isContraAcc2;

                var contraAccountNo = isContraAcc1 ? "60555504645" : "60548252992";

                // Resolve Target Ledger
                var targetLedger = "Suspence Account";
                
                if (isTeraSoftware) {
                    targetLedger = "TERA SOFTWARE LIMITED - UPPCL";
                } else if (isIndraprastha) {
                    targetLedger = "INDRAPRASTHA GAS LIMITED";
                } else if (isSelfOrCash) {
                    targetLedger = "Cash";
                } else if (isClixCapital) {
                    targetLedger = "CLIX CAPITAL SERVICES PRIVATE LIMITED";
                } else if (isUgroCapital) {
                    targetLedger = "UGRO CAPITAL LIMITED";
                } else if (isBankCharges) {
                    targetLedger = "Bank Charges";
                } else if (isBankCommission) {
                    targetLedger = "Bank Commission";
                } else if (isOriental) {
                    targetLedger = "The oriental insurance company limited";
                } else if (isBomtl) {
                    targetLedger = "Bank of Maharashtra (Term Loan A/c)";
                } else if (isBombvl) {
                    targetLedger = "BOM (Bolero Vehicle Loan)";
                } else if (isBomevvl) {
                    targetLedger = "BOM (Mahindra EV-Vehicle Loan)";
                } else if (isBomvl) {
                    targetLedger = "BOM (Vehicle Loan)";
                } else if (isBomtlnew) {
                    targetLedger = "Term Loan BOM-New";
                } else if (isknl) {
                    targetLedger = "K Nagalaxmi";
                } else if (isspac) {
                    targetLedger = "Salaries Payables Account";
                } else if (isdbintr) {
                    targetLedger = "Bank Interest Charges";
                } else if (isContra) {
                    targetLedger = contraAccountNo;
                } else if (tallyLedgerVal !== "" && tallyLedgerVal.toUpperCase() !== "N/A") {
                    targetLedger = tallyLedgerVal;
                } else if (beneficiary !== "" && beneficiary.toUpperCase() !== "N/A") {
                    targetLedger = beneficiary;
                }

                // Check for "babji kollipara"
                if (targetLedger.toLowerCase().includes("babji kollipara") || 
                    tallyLedgerVal.toLowerCase().includes("babji kollipara") || 
                    beneficiary.toLowerCase().includes("babji kollipara") || 
                    particulars.toLowerCase().includes("babji kollipara")) {
                    targetLedger = "KBabji";
                }
                
                // Check for "nagalakshmi kollipara"
                if (targetLedger.toLowerCase().includes("nagalakshmi kollipara") || 
                    tallyLedgerVal.toLowerCase().includes("nagalakshmi kollipara") || 
                    beneficiary.toLowerCase().includes("nagalakshmi kollipara") || 
                    particulars.toLowerCase().includes("nagalakshmi kollipara")) {
                    targetLedger = "K Nagalaxmi";
                }

                var defaultBankLedger = "Bank of Maharashtra CC A/C";
                var isSpecialMatched = isTeraSoftware || isIndraprastha || isClixCapital || isUgroCapital || isBankCharges || isBankCommission || isOriental || isBomtl || isBombvl || isBomevvl || isBomvl || isBomtlnew || isknl || isspac || isdbintr;

                if (debit > 0) {
                    var debitLedger = normalizeLedgerName(defaultBankLedger);
                    var creditLedger = normalizeLedgerName(targetLedger);
                    ...
                    } else if (isContra && !isSpecialMatched) {
                        tallyRows.push([
                            companyName,
                            "Contra - Debit",
                            voucherDate,
                            ...
                            debit,
                            creditLedger,
                            tallyLedgerVal !== "" && tallyLedgerVal.toUpperCase() !== "N/A" ? tallyLedgerVal : ("BOM to " + contraAccountNo)
                        ]);
                    } else {
                        tallyRows.push([
                            companyName,
                    ...
                    var debitLedger = normalizeLedgerName(targetLedger);
                    var creditLedger = normalizeLedgerName(defaultBankLedger);
                    ...
                    if (isSelfOrCash) {
                        ...
                        tallyRows.push([
                            ...
                            credit,
                            creditLedger,
                            tallyLedgerVal !== "" && tallyLedgerVal.toUpperCase() !== "N/A" ? tallyLedgerVal : ("Transfer from " + contraAccountNo)
                        ]);
                    } else {
                        tallyRows.push([
                            companyName,
            ...
            if (tallyRows.length <= 1) {
                alert('No valid debit/credit records found for export.');
                return;
            }

            // Determine Account Number Part
            var accNamePart = "BOM";
            if (selectedAccFilter !== "") {
                accNamePart = selectedAccFilter;
            } else if (exportedAccounts.size === 1) {
                accNamePart = Array.from(exportedAccounts)[0];
            } else if (exportedAccounts.size > 1) {
                accNamePart = "All_BOM_Accounts";
            }

            // Determine Month and Year Part
            var dateFromVal = (document.getElementById('filterDateFrom') ? document.getElementById('filterDateFrom').value : '').trim();
            var periodPart = "";

            if (dateFromVal) {
                var dObj = parseDateString(dateFromVal);
                if (dObj) periodPart = monthNames[dObj.getMonth()] + "_" + dObj.getFullYear();
            }
            
            if (!periodPart && exportedMonthYears.size > 0) {
                periodPart = Array.from(exportedMonthYears).join("_");
            }
            
            if (!periodPart) {
                var now = new Date();
                periodPart = monthNames[now.getMonth()] + "_" + now.getFullYear();
            }

            var finalFileName = "Tally_Ledger_" + accNamePart + "_" + periodPart + ".xlsx";

            var ws = XLSX.utils.aoa_to_sheet(tallyRows);
            ws['!cols'] = [
                { wch: 38 },
                { wch: 18 },
                { wch: 16 },
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Tally_Ledger");
        ...
        function exportExcel() {
            ...
            var wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "BOM Statement");
            XLSX.writeFile(wb, "bom_transactions_report.xlsx");
        }
        ...
        function exportCsv() {
            ...
            a.href = url;
            a.download = 'bom_transactions_report.csv';
        ...
        function exportPdf() {
            ...
            var html = '<!doctype html><html><head><title>BOM Transactions Report</title>' +
                ...
                '</head><body>';
            html += '<h3 style="color:#0f172a;">BOM Transactions Report</h3>';
